...
const uint32_t OLED_I2C_HZ = 400000;          // ความเร็ว I2C

// ส่งคำสั่งหนึ่งไบต์ไปที่จอ
bool oledCmd(uint8_t c) {
...
// เริ่มเพอริเฟอรัล I2C ของ ESP32 ใหม่
// ใช้ตอนบัสค้าง ซึ่งเกิดเมื่อรายการถูกตัดกลางคัน เช่นสายหลุดชั่วขณะขณะกำลังส่ง
// ฝั่งที่ค้างคือตัวส่งของ ESP32 เอง จอไม่เกี่ยว เสียบสายกลับก็ไม่ฟื้นถ้าไม่เริ่มใหม่
void restartI2C() {
  Serial.println("ตัวส่ง I2C ค้าง กำลังเริ่มใหม่");
  Wire.end();
...
bool oledResync() {
  // ถ้าคำสั่งแรกไม่ผ่าน แปลว่าบัสไปต่อไม่ได้ ไม่ต้องเสียเวลารอหมดเวลาอีก 19 ครั้ง
  if (!oledCmd(0xD5)) return false;
  oledCmd(0x80);   // ความถี่สแกน
...
void pushFrame() {
  ...
  // ปลดชิปจอออกจากสภาพค้างกลางคำสั่ง ถ้าบังเอิญค้างอยู่
  if (!oledResync()) { restartI2C(); return; }

  // ล้มติดกันกี่ก้อนถึงเลิกส่งเฟรมนี้แล้วเริ่มบัสใหม่
  //
  // สำคัญที่ต้องเลิกให้เร็ว เพราะตอนบัสค้าง ทุกรายการจะรอจนหมดเวลาก่อนยอมแพ้
  // หนึ่งเฟรมมีเกือบ 90 รายการ ถ้าดันทุรังส่งจนครบจะกินเวลาหลายวินาทีต่อเฟรม
  // กว่าจะกลับมาวาดได้ใหม่จึงนานมาก ทั้งที่การเริ่มบัสใหม่ใช้เวลาไม่ถึงมิลลิวินาที
  const int ABORT_AFTER = 4;
  int streak = 0;
  ...
  for (uint8_t page = 0; page < 64 / 8; page++) {
    ...
    for (int x = 0; x < 128; x += OLED_CHUNK) {
      ...
      if (Wire.endTransmission() == 0) {
        streak = 0;
      } else if (++streak >= ABORT_AFTER) {
        restartI2C();   // เฟรมนี้ทิ้งไป เฟรมหน้าค่อยว่ากันใหม่
        return;
      }
    }
  }
