...
void pushFrame() {
  ...
  int fails = 0;  // จำนวนก้อนข้อมูลที่ส่งไม่สำเร็จในเฟรมนี้
  int sent = 0;   // จำนวนก้อนที่พยายามส่งทั้งหมด
  ...
  // ถ้าส่งไม่ผ่านเลยสักก้อนเดียวทั้งเฟรม แปลว่าตัวส่ง I2C ของ ESP32 ค้าง ไม่ใช่จอ
  //
  // เกิดได้เมื่อรายการถูกตัดกลางคัน เช่นตอนดึงสายออกขณะกำลังส่ง หรือหน้าสัมผัสหลุด
  // ชั่วขณะ เพอริเฟอรัลจะค้างอยู่อย่างนั้นถาวร เสียบสายกลับก็ไม่ฟื้นเอง
  //
  // เงื่อนไขตั้งไว้เข้มว่าต้องล้มครบทุกก้อน เพราะถ้าล้มบางก้อนอาจเป็นสัญญาณรบกวน
  // ชั่วครู่ซึ่งไม่ควรไปรื้อบัสที่ยังใช้ได้อยู่
  //
  // ต่างจากตัวตรวจจับรุ่นก่อนตรงที่อันนี้ดูผลของการส่งข้อมูลจริงที่ใช้งานอยู่
  // ไม่ใช่ยิงคำสั่งทดสอบแยกต่างหาก ซึ่งเคยรายงานว่าจอเสียทั้งที่จอทำงานได้ปกติ
  if (sent > 0 && fails == sent) {
