...
void loop() {
  ...
  if (Serial.available() > 0) {
    char qrCommand = Serial.read(); // อ่านตัวอักษรเดียวจาก Python
    
    if (qrCommand == 'D') { // ถ้าเจอ DOG
      Serial.println(">>>> QR DETECTED: DOG! STOPPING 5 SEC <<<<");
      brakeMotor();       // ใช้ฟังก์ชันเบรกที่คุณมีอยู่แล้ว
      delay(5000);        // หยุดค้างไว้ 5 วินาที
